...
public class MessageDaoImpl implements MessageDao {
    ...
    public void save(Message message, User user) {
        String sql = "INSERT INTO messages (id, text, date_time, link, link_title, link_description, link_cover, user_id) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        jdbcTemplate.update(sql, message.getId(), message.getText(), LocalDateTime.now(),
                message.getLink(), message.getLinkTitle(), message.getLinkDescription(), message.getLinkCover(), user.getId());
    }
